...
export default function AdminPage() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#1A0A12',display:'flex',flexDirection:'column'}}>
      {/* Header */}
      ...
      <div style={{display:'flex',flex:1}}>
        {/* Sidebar */}
        <div style={{width:'220px',background:'#2C1020',borderRight:'1px solid #4A1030',padding:'24px 0'}}>
          {navItems.map(item => (
            <button key={item.id} onClick={()=>setActivePage(item.id)}
              style={{width:'100%',padding:'14px 24px',background:activePage===item.id?'#6B2D4E':'none',border:'none',cursor:'pointer',display:'flex',alignItems:'center',gap:'10px',color:activePage===item.id?'#FAF0E6':'rgba(250,240,230,0.5)',fontSize:'14px',fontWeight:activePage===item.id?'700':'400',textAlign:'left'}}>
              <span>{item.icon}</span>
              {item.label}
            </button>
          ))}
        ...
        <div style={{flex:1,padding:'32px',overflowY:'auto'}}>
          ...
          {activePage === 'overview' && (
            <div>
              <h1 style={{color:'#D4AF7A',fontSize:'24px',fontWeight:'800',marginBottom:'8px'}}>Platform Overview</h1>
              <p style={{color:'rgba(250,240,230,0.5)',fontSize:'14px',marginBottom:'32px'}}>Welcome back, Super Admin.</p>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'20px',marginBottom:'32px'}}>
                {[
                  ...
                  <div key={i} style={{background:'#2C1020',border:`1px solid ${stat.color}`,borderRadius:'16px',padding:'24px'}}>
                    ...
                    <div style={{color:'rgba(250,240,230,0.5)',fontSize:'12px',marginBottom:'4px'}}>{stat.label}</div>
                    <div style={{color:'#FAF0E6',fontSize:'28px',fontWeight:'800'}}>{stat.value}</div>
                  </div>
                ))}
              </div>

              {/* Recent Groups */}
              <div style={{background:'#2C1020',border:'1px solid #4A1030',borderRadius:'16px',padding:'24px'}}>
                <h2 style={{color:'#D4AF7A',fontSize:'18px',fontWeight:'700',marginBottom:'16px'}}>Recent Groups</h2>
                {groups.length === 0 ? (
                  <p style={{color:'rgba(250,240,230,0.5)',fontSize:'14px'}}>No groups yet.</p>
                ) : (
                  groups.slice(0, 5).map((g, i) => (
                    <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'12px 0',borderBottom:'1px solid #4A1030'}}>
                      <span style={{color:'#FAF0E6',fontSize:'14px'}}>{g.id}</span>
                      <span style={{color:'#D4AF7A',fontSize:'12px'}}>Active</span>
                    </div>
                  ))
                )}
              </div>
          ...
          {activePage === 'groups' && (
            <div>
              ...
              {groups.length === 0 ? (
                ...
                groups.map((g, i) => (
                  <div key={i} style={{background:'#2C1020',border:'1px solid #4A1030',borderRadius:'12px',padding:'20px',marginBottom:'12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div>
                      <div style={{color:'#FAF0E6',fontSize:'16px',fontWeight:'700'}}>{g.id}</div>
                      <div style={{color:'rgba(250,240,230,0.5)',fontSize:'12px',marginTop:'4px'}}>Group ID</div>
                    </div>
            ...
            <div>
              ...
              {members.length === 0 ? (
                ...
                members.map((m, i) => (
                  <div key={i} style={{background:'#2C1020',border:'1px solid #4A1030',borderRadius:'12px',padding:'20px',marginBottom:'12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div>
                      <div style={{color:'#FAF0E6',fontSize:'16px',fontWeight:'700'}}>{m.name || 'Unknown'}</div>
                      <div style={{color:'rgba(250,240,230,0.5)',fontSize:'12px',marginTop:'4px'}}>Group: {m.groupId}</div>
                    </div>
                    <div style={{color:'#D4AF7A',fontSize:'14px',fontWeight:'700'}}>{m.id}</div>
                  </div>
                ))
              )}
            </div>
            ...
            <div>
              ...
              {payments.length === 0 ? (
                ...
                payments.map((p, i) => (
                  <div key={i} style={{background:'#2C1020',border:'1px solid #4A1030',borderRadius:'12px',padding:'20px',marginBottom:'12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div>
                      <div style={{color:'#FAF0E6',fontSize:'16px',fontWeight:'700'}}>${p.amount || 0}</div>
                      <div style={{color:'rgba(250,240,230,0.5)',fontSize:'12px',marginTop:'4px'}}>Group: {p.groupId}</div>
                    </div>
          ...
          {activePage === 'settings' && (
            <div>
              <h1 style={{color:'#D4AF7A',fontSize:'24px',fontWeight:'800',marginBottom:'24px'}}>Platform Settings</h1>
              <div style={{background:'#2C1020',border:'1px solid #4A1030',borderRadius:'16px',padding:'24px'}}>
                <div style={{marginBottom:'20px'}}>
                  <div style={{color:'rgba(250,240,230,0.5)',fontSize:'12px',marginBottom:'4px'}}>Super Admin Email</div>
                  <div style={{color:'#FAF0E6',fontSize:'16px'}}>{SUPER_ADMIN_EMAIL}</div>
                </div>
                <div style={{marginBottom:'20px'}}>
                  <div style={{color:'rgba(250,240,230,0.5)',fontSize:'12px',marginBottom:'4px'}}>Platform Fee</div>
                  <div style={{color:'#D4AF7A',fontSize:'16px',fontWeight:'700'}}>0.5% per distribution</div>
                </div>
                <div style={{marginBottom:'20px'}}>
                  <div style={{color:'rgba(250,240,230,0.5)',fontSize:'12px',marginBottom:'4px'}}>Platform Domain</div>
                  <div style={{color:'#FAF0E6',fontSize:'16px'}}>tarsyn-app.com</div>
                </div>
                <div>
                  <div style={{color:'rgba(250,240,230,0.5)',fontSize:'12px',marginBottom:'4px'}}>Version</div>
                  <div style={{color:'#FAF0E6',fontSize:'16px'}}>1.0.0</div>
                </div>
              </div>
            </div>
          )}
