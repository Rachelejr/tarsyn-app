  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAF0E6', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <p style={{ color: '#7A5068', fontSize: '15px', margin: '0 0 32px' }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '24px', marginBottom: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div>
              <label style={{ color: '#7A5068', fontSize: '13px', display: 'block', marginBottom: '6px' }}>Amount Due</label>
              <input value={amount} onChange={e => setAmount(e.target.value)}
                placeholder="e.g. 150"
                style={{ width: '100%', padding: '10px 14px', borderRadius: '10px', border: '1px solid #E8D5C4', fontSize: '14px', boxSizing: 'border-box' }} />
            </div>
            <div>
              <label style={{ color: '#7A5068', fontSize: '13px', display: 'block', marginBottom: '6px' }}>Due Date</label>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <p style={{ color: '#7A5068', fontSize: '14px', margin: 0 }}>{members.length} members with email</p>
          <button onClick={sendAll}
            style={{ background: '#6B2D4E', color: '#FAF0E6', padding: '10px 24px', borderRadius: '10px', border: 'none', fontWeight: 700, cursor: 'pointer', fontSize: '14px' }}>
            📨 Send to All
          </button>
        </div>
        ...
        {members.length === 0 ? (
          <div style={{ background: 'white', borderRadius: '16px', padding: '32px', textAlign: 'center' }}>
            <p style={{ color: '#7A5068' }}>No members with email addresses found.</p>
          </div>
        ) : (
            ...
            {members.sort((a, b) => a.position - b.position).map(m => (
              <div key={m.id} style={{ background: 'white', borderRadius: '14px', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  <div style={{ width: '40px', height: '40px', background: '#6B2D4E', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#D4AF7A', fontWeight: 800, fontSize: '14px' }}>
                    {m.name?.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <p style={{ color: '#2C1A3E', fontWeight: 700, margin: '0 0 2px', fontSize: '15px' }}>{m.name}</p>
                    <p style={{ color: '#7A5068', fontSize: '13px', margin: 0 }}>{m.email} · #{m.position}</p>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: sent.includes(m.id) ? '#2E7D32' : '#FAF0E6',
