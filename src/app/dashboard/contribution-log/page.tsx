...
function RegisterContent() {
  ...
  const handleStatusChange = async (memberId: string, cycle: number, newStatus: PaymentStatus) => {
    ...
    try {
      if (existing) {
        const updatedAmount = firestoreStatus === 'confirmed' ? (group?.amountPerMember || existing.amount || 0) : existing.amount;
  ...
  const recentActivity = [
    group?.createdAt && { icon: '', text: 'Group created', date: group.name },
    ...
    ...payments.slice(-3).reverse().map(p => {
      ...
      const statusText = label ? label.charAt(0).toUpperCase() + label.slice(1) : (p.status || 'Updated');
      const text = p.amount > 0
        ? `Payment recorded - ${p.amount} ${p.currency || ''}`.trim()
        : `Payment status updated - ${statusText}`;
      return { icon: '', text, date: p.paymentDate };
    }),
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '20px 28px 50px' }}>
          ...
          {statusError && (
            <div className="fade-up no-print" style={{ background: '#FDEAEA', border: '1px solid #E8B4B4', borderRadius: '10px', padding: '10px 16px', marginBottom: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <p style={{ color: '#8B2D2D', fontSize: '12.5px', margin: 0 }}> {statusError}</p>
              <button onClick={() => setStatusError('')} style={{ background: 'none', border: 'none', color: '#8B2D2D', cursor: 'pointer', fontSize: '14px', fontWeight: 700 }}>x</button>
            </div>
          )}
