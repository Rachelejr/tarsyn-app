  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: '16px',
        padding: '16px 18px',
        boxShadow: '0 4px 16px rgba(110,147,172,0.08)',
        display: 'flex',
        alignItems: 'center',
        gap: '12px',
    ...
    >
      ...
      <div>
        <p style={{ color: '#7FA0B8', fontSize: '10px', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '1.1px', fontWeight: 700 }}>{label}</p>
        <p style={{ color: '#4A6B85', fontSize: '21px', fontWeight: 800, margin: 0, letterSpacing: '-0.5px' }}>
    ...
    <div style={{ minHeight: '100vh', background: '#FBEEDD', fontFamily: 'Inter, sans-serif' }}>
      <style>{`
        ...
        .stat-card:hover {
          ...
          box-shadow: 0 8px 22px rgba(110,147,172,0.14) !important;
        }
        .panel-card:hover {
          box-shadow: 0 6px 22px rgba(110,147,172,0.10) !important;
        }
        .action-card:hover {
      ...
      {editingGroup && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(38,64,79,0.45)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="modal-fade" style={{ background: 'white', borderRadius: '20px', padding: '32px', maxWidth: '400px', width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
            <h3 style={{ color: '#6E93AC', fontSize: '18px', fontWeight: 700, margin: '0 0 16px' }}>Edit Group Name</h3>
            <input
              value={newGroupName}
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => { setEditingGroup(null); setNewGroupName(''); }} className="btn-action"
                style={{ flex: 1, padding: '12px', background: 'transparent', color: '#6E93AC', border: '2px solid #6E93AC', borderRadius: '10px', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={handleSaveGroupName} disabled={savingGroup} className="btn-action"
                style={{ flex: 1, padding: '12px', background: '#6E93AC', color: '#FBEEDD', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>
      ...
      <nav style={{
        background: 'linear-gradient(135deg, #6E93AC 0%, #4A6B85 100%)',
        padding: '12px 28px',
        display: 'grid',
        ...
        <div onClick={() => router.push('/')} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', justifySelf: 'start' }}>
          <div style={{ width: '32px', height: '32px', background: 'linear-gradient(135deg,#E9C77B,#C9974D)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: '#6E93AC', fontSize: '13px', boxShadow: '0 3px 10px rgba(233,199,123,0.4)' }}>T</div>
          <div>
        ...
        <div className="fade-up" style={{ textAlign: 'center', justifySelf: 'center', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '20px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '12px', marginBottom: '18px' }}>
          <StatCard label="Total Members" value={members.length} icon="👥" gradient="linear-gradient(135deg,#6E93AC,#4A6B85)" glow="rgba(110,147,172,0.35)" delay={0} />
        ...
        <div className="panel-card fade-up" style={{ background: 'white', borderRadius: '16px', padding: '18px 20px', boxShadow: '0 2px 14px rgba(110,147,172,0.06)', marginBottom: '14px' }}>
          <h3 style={{ color: '#6E93AC', fontSize: '15px', fontWeight: 700, margin: '0 0 12px' }}>🏘️ My Groups</h3>
          {groups.length === 0 ? (
            <p style={{ color: '#7FA0B8', fontSize: '13px' }}>No groups yet. <span onClick={() => router.push('/dashboard/create-tontine')} style={{ color: '#6E93AC', fontWeight: 700, cursor: 'pointer', textDecoration: 'underline' }}>Create your first group</span></p>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '10px' }}>
              {groups.map((g, i) => (
                <div key={i} style={{ background: '#FBEEDD', borderRadius: '12px', padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap' }}>
                  <div>
                    <p style={{ color: '#6E93AC', fontWeight: 700, fontSize: '14px', margin: '0 0 2px' }}>{g.name}</p>
                    <p style={{ color: '#7FA0B8', fontSize: '11px', margin: 0 }}>{g.frequency} · {g.status}</p>
                  </div>
                  <button onClick={() => { setEditingGroup(g); setNewGroupName(g.name); }} className="btn-action"
                    style={{ background: '#6E93AC', color: '#FBEEDD', border: 'none', borderRadius: '8px', padding: '5px 11px', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>
                    ✏️ Edit
                  </button>
                </div>
              ))}
        ...
        <div className="panel-card fade-up" style={{ background: 'white', borderRadius: '16px', padding: '18px 20px', boxShadow: '0 2px 14px rgba(110,147,172,0.06)', marginBottom: '14px' }}>
          <h3 style={{ color: '#6E93AC', fontSize: '15px', fontWeight: 700, margin: '0 0 12px' }}>👥 Member Management</h3>
          {members.length === 0 ? (
            <p style={{ color: '#7FA0B8', fontSize: '13px' }}>No members yet.</p>
          ) : (
                ...
                <thead>
                  <tr style={{ borderBottom: '2px solid #FBEEDD' }}>
                    {['#', 'TYN-ID', 'Name', 'Payout Date', 'Status', 'Actions'].map(h => (
                      <th key={h} style={{ textAlign: 'left', padding: '6px 10px', color: '#7FA0B8', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '1px' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {members.sort((a, b) => a.position - b.position).map((m, i) => (
                    <tr key={m.id} className="row-hover" style={{ borderBottom: '1px solid #FBEEDD', transition: 'background 0.15s ease' }}>
                      <td style={{ padding: '10px 10px', color: '#6E93AC', fontWeight: 700, fontSize: '13px' }}>#{m.position}</td>
                      <td style={{ padding: '10px 10px', color: '#7FA0B8', fontFamily: 'monospace', fontSize: '12px' }}>{m.tynId}</td>
                      <td style={{ padding: '10px 10px', color: '#4A6B85', fontWeight: 600, fontSize: '13px' }}>{m.name}</td>
                      <td style={{ padding: '10px 10px', color: '#7FA0B8', fontSize: '12px' }}>{m.payoutDate || '—'}</td>
        ...
        {pendingProofs.length > 0 && (
          <div className="panel-card fade-up" style={{ background: 'white', borderRadius: '16px', padding: '18px 20px', boxShadow: '0 2px 14px rgba(110,147,172,0.06)', marginBottom: '14px' }}>
            <h3 style={{ color: '#6E93AC', fontSize: '15px', fontWeight: 700, margin: '0 0 4px' }}>📎 Payment Proofs</h3>
            <p style={{ color: '#7FA0B8', fontSize: '12px', margin: '0 0 12px' }}>{pendingProofs.length} proof(s) waiting for validation</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {pendingProofs.map((p, i) => (
                <div key={p.id} style={{ background: '#FBEEDD', borderRadius: '12px', padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
                  <div>
                    <p style={{ color: '#6E93AC', fontWeight: 700, fontSize: '13px', margin: '0 0 2px' }}>{p.memberName}</p>
                    <p style={{ color: '#7FA0B8', fontSize: '11px', margin: 0 }}>{p.amount} {p.currency} · {p.paymentDate} · {p.paymentMethod}</p>
                  </div>
        ...
        <div className="panel-card fade-up" style={{ background: 'white', borderRadius: '16px', padding: '18px 20px', boxShadow: '0 2px 14px rgba(110,147,172,0.06)', marginBottom: '14px' }}>
          <h3 style={{ color: '#6E93AC', fontSize: '15px', fontWeight: 700, margin: '0 0 12px' }}>💰 Recent Contributions</h3>
          {payments.length === 0 ? (
            <p style={{ color: '#7FA0B8', fontSize: '13px' }}>No payments recorded yet.</p>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                    ...
                    <tr key={p.id} className="row-hover" style={{ borderBottom: '1px solid #FBEEDD', transition: 'background 0.15s ease' }}>
                      <td style={{ padding: '10px 10px' }}>
                        <a href={`/receipt/${p.receiptNumber}`} target="_blank" rel="noreferrer"
                          style={{ color: '#6E93AC', fontFamily: 'monospace', fontSize: '11px', fontWeight: 700, textDecoration: 'underline' }}>
                          {p.receiptNumber || '—'}
                        </a>
                      </td>
                      <td style={{ padding: '10px 10px', color: '#4A6B85', fontWeight: 600, fontSize: '13px' }}>{p.memberName}</td>
                      <td style={{ padding: '10px 10px', color: '#2E7D32', fontWeight: 700, fontSize: '13px' }}>{p.amount} {p.currency}</td>
                      <td style={{ padding: '10px 10px', color: '#7FA0B8', fontSize: '12px' }}>{p.paymentMethod}</td>
                      <td style={{ padding: '10px 10px', color: '#7FA0B8', fontSize: '12px' }}>{p.paymentDate}</td>
                      <td style={{ padding: '10px 10px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '12px', paddingBottom: '24px' }}>
          {[
            ...
            <div key={i} className="action-card" onClick={() => router.push(a.path)}
              ...
              <p style={{ color: '#6E93AC', fontWeight: 700, fontSize: '14px', margin: 0 }}>{a.title}</p>
            </div>
          ))}
        </div>
      </div>
