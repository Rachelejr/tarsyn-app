    ...
    <div style={{ minHeight: '100vh', background: C.creme, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '24px', padding: '48px', maxWidth: '520px', width: '100%', boxShadow: '0 8px 32px rgba(107,45,78,0.12)', textAlign: 'center' }}>
        ...
        <div style={{ background: C.creme, borderRadius: '16px', padding: '20px', marginBottom: '24px', textAlign: 'left' }}>
          {[
            { label: 'Tontine Code', value: savedGroup.tontineCode },
            ...
            <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', paddingBottom: '10px', borderBottom: `1px solid ${C.roseClair}` }}>
              ...
              <span style={{ color: C.bordeaux, fontWeight: '700', fontSize: '13px', fontFamily: 'monospace' }}>{item.value}</span>
            </div>
          ))}
          <div style={{ marginTop: '8px' }}>
            <p style={{ color: C.texteGris, fontSize: '12px', margin: '0 0 6px' }}>Invite Link</p>
            <p style={{ color: C.bordeaux, fontSize: '12px', wordBreak: 'break-all', fontWeight: '600' }}>{savedGroup.inviteLink}</p>
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
          <button onClick={() => navigator.clipboard.writeText(savedGroup.inviteLink)}
            style={{ background: C.dore, color: C.bordeaux, padding: '12px', borderRadius: '12px', border: 'none', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
            Copy Link
          </button>
          <button onClick={() => router.push('/dashboard/add-member')}
            style={{ background: C.roseClair, color: C.bordeaux, padding: '12px', borderRadius: '12px', border: 'none', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
            Invite Members
          </button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <button onClick={() => router.push('/dashboard')}
            style={{ background: C.bordeaux, color: 'white', padding: '12px', borderRadius: '12px', border: 'none', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
            Dashboard
          </button>
          <button onClick={() => { setSavedGroup(null); setRegion(''); setCustomName(''); setNumMembers(''); setContribution(''); setStartDate(''); setEmailList([]); }}
            style={{ background: 'white', color: C.bordeaux, padding: '12px', borderRadius: '12px', border: `2px solid ${C.bordeaux}`, fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
            Create Another
          </button>
        </div>
      </div>
      ...
      <div style={{ width: '100%', maxWidth: '720px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '0 0 20px 20px', border: `1px solid ${C.roseMoyen}`, borderTop: 'none', boxShadow: '0 8px 40px rgba(107,45,78,0.1)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '32px 40px' }}>
            ...
            {totalPool > 0 && (
              <div style={{ background: C.roseClair, borderRadius: '12px', padding: '16px', marginBottom: '18px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                <div style={{ textAlign: 'center' }}>
                  <p style={{ color: C.texteGris, fontSize: '12px', margin: '0 0 4px' }}>Total Pool</p>
                  <p style={{ color: C.bordeaux, fontWeight: '800', fontSize: '18px', margin: 0 }}>{totalPool} {currency}</p>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <p style={{ color: C.texteGris, fontSize: '12px', margin: '0 0 4px' }}>Per Member</p>
                  <p style={{ color: C.bordeaux, fontWeight: '800', fontSize: '18px', margin: 0 }}>{contrib} {currency}</p>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <p style={{ color: C.texteGris, fontSize: '12px', margin: '0 0 4px' }}>Total Cycle</p>
                  <p style={{ color: C.bordeaux, fontWeight: '800', fontSize: '18px', margin: 0 }}>{totalCycle} {currency}</p>
                </div>
              </div>
            )}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {FREQUENCIES.map(f => (
                  <button key={f} onClick={() => setFrequency(f)}
                    style={{ padding: '8px 18px', borderRadius: '20px', border: `2px solid ${frequency === f ? C.bordeaux : C.roseMoyen}`, background: frequency === f ? C.bordeaux : 'white', color: frequency === f ? 'white' : C.texteGris, cursor: 'pointer', fontSize: '13px', fontWeight: '500' }}>
                    {f}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: '18px' }}>
              <FieldLabel icon="📋" label="Rules Template" />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '18px' }}>
              <div>
                <FieldLabel icon="👁️" label="Organizer Visibility" />
                <select value={adminVisibility} onChange={e => setAdminVisibility(e.target.value)} style={inp}>
                  <option>Show Organizer</option>
                  <option>Hidden Organizer</option>
                </select>
              </div>
              <div>
                <FieldLabel icon="🌐" label="Interface Language" />
                <select value={language} onChange={e => setLanguage(e.target.value)} style={inp}>
                  {LANGUAGES.slice(0, 20).map(l => <option key={l} value={l}>{l}</option>)}
                </select>
              </div>
            </div>
            ...
            <button onClick={handleSubmit} disabled={saving}
              style={{ width: '100%', padding: '15px', background: saving ? C.roseMoyen : C.bordeaux, color: saving ? C.texteGris : 'white', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '700', cursor: saving ? 'not-allowed' : 'pointer', boxShadow: saving ? 'none' : `0 4px 20px rgba(107,45,78,0.35)` }}>
              {saving ? '⏳ Creating...' : '🚀 Create Tontine'}
            </button>
