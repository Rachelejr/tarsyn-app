...
export default function AddMember() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setGroupId(gId);
      const mq = query(collection(db, 'members'), where('groupId', '==', gId));
  ...
  const validate = () => {
    ...
    if (new Date(payoutDate) < new Date()) { setError('Payout date cannot be in the past.'); return false; }
    const dupPosition = existingMembers.find(m => m.position === parseInt(position));
    if (dupPosition) { setError(`Position ${position} is already assigned to ${dupPosition.name}.`); return false; }
    if (email) {
        ...
        <div style={{background:'white',borderRadius:'24px',padding:'40px',boxShadow:'0 8px 32px rgba(107,45,78,0.12)'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'16px'}}>
            <div>
              ...
              <input value={expectedAmount} onChange={e => setExpectedAmount(e.target.value)} type="number" min="0"
                placeholder="0.00"
                style={{width:'100%',padding:'12px 14px',border:'1.5px solid #E8D5E0',borderRadius:'10px',fontSize:'14px',outline:'none',boxSizing:'border-box'}}/>
            </div>
            <div>
