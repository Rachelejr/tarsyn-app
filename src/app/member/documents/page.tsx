  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FBEEDD', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
        <h1 style={{ color: '#6E93AC', fontSize: '28px', fontWeight: 800, margin: '0 0 4px' }}>📁 Document Center</h1>
        <p style={{ color: '#6E93AC', fontSize: '15px', margin: '0 0 32px' }}>{groupName} · Upload and manage group documents</p>

        <div style={{ background: 'white', borderRadius: '16px', padding: '24px', marginBottom: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <h3 style={{ color: '#6E93AC', fontWeight: 700, margin: '0 0 16px' }}>Upload Document</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            <div>
              <label style={{ color: '#6E93AC', fontSize: '13px', display: 'block', marginBottom: '6px' }}>Category</label>
            ...
            <div>
              <label style={{ color: '#6E93AC', fontSize: '13px', display: 'block', marginBottom: '6px' }}>File</label>
          ...
          {uploading && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                <span style={{ color: '#6E93AC', fontSize: '13px' }}>Uploading...</span>
                <span style={{ color: '#6E93AC', fontSize: '13px', fontWeight: 700 }}>{progress}%</span>
              </div>
              <div style={{ background: '#FBEEDD', borderRadius: '999px', height: '8px' }}>
                <div style={{ background: '#6E93AC', width: `${progress}%`, height: '8px', borderRadius: '999px', transition: 'width 0.3s' }} />
              </div>
            </div>
          )}
        </div>
        ...
        {categories.map(cat => {
          ...
          return (
            <div key={cat} style={{ marginBottom: '24px' }}>
              <h3 style={{ color: '#6E93AC', fontWeight: 700, fontSize: '16px', margin: '0 0 12px' }}>{cat}</h3>
                ...
                {catDocs.map((d: any) => (
                  <div key={d.id} style={{ background: 'white', borderRadius: '14px', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                      ...
                      <div>
                        <p style={{ color: '#4A6B85', fontWeight: 700, margin: '0 0 2px', fontSize: '14px' }}>{d.name}</p>
                        <p style={{ color: '#6E93AC', fontSize: '12px', margin: 0 }}>{formatSize(d.size)} · {d.category}</p>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <a href={d.url} target="_blank" rel="noreferrer"
                        style={{ background: '#6E93AC', color: '#FBEEDD', padding: '7px 14px', borderRadius: '8px', fontSize: '12px', fontWeight: 600, textDecoration: 'none' }}>
                        ⬇️ Download
                      </a>
        ...
        {docs.length === 0 && !uploading && (
          <div style={{ background: 'white', borderRadius: '16px', padding: '48px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#6E93AC', fontSize: '15px' }}>No documents yet. Upload your first file!</p>
          </div>
        )}
      </div>
    </div>
