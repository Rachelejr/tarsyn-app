...
export default function MemberPage() {
  ...
  const handleSendContact = async () => {
    ...
    try {
      await addDoc(collection(db, 'messages'), {
        from: user.uid, tynId: MEMBER_DATA.tynId,
        subject: contactSubject, message: contactMessage,
        createdAt: serverTimestamp(), read: false,
      });
      setContactSent(true); setContactSubject(''); setContactMessage('');
  ...
  const handleSendComment = async () => {
    ...
    try { await addDoc(collection(db, 'comments'), { tynId: MEMBER_DATA.tynId, text: comment, createdAt: serverTimestamp() }); } catch (e) { console.error(e); }
  ...
  const handleSendRating = async () => {
    if (!rating) return;
    try { await addDoc(collection(db, 'ratings'), { tynId: MEMBER_DATA.tynId, rating, comment: ratingComment, createdAt: serverTimestamp() }); } catch (e) { console.error(e); }
    setRatingSent(true);
  };
  ...
  const inp: React.CSSProperties = { width: '100%', padding: '10px 14px', border: '1.5px solid #D9C0CC', borderRadius: '8px', fontSize: '14px', outline: 'none', boxSizing: 'border-box', color: '#2C1A24', background: '#FAF0E6' };
  const btn = (extra = {}): React.CSSProperties => ({ padding: '10px 20px', background: '#6B2D4E', color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', ...extra });
      ...
      <main style={{ marginLeft: '230px', flex: 1, minHeight: '100vh' }}>
        <div style={{ background: 'white', borderBottom: '1px solid #D9C0CC', padding: '0 28px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 40 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            {unreadCount > 0 && <span onClick={() => setActivePage('notifications')} style={{ fontSize: '11px', fontWeight: '700', background: '#C0392B', color: 'white', padding: '4px 10px', borderRadius: '20px', cursor: 'pointer' }}>🔔 {unreadCount}</span>}
        ...
        <div style={{ padding: '24px' }}>
            ...
            <>
              <div style={{ background: 'white', border: '1px solid #D9C0CC', borderRadius: '12px', marginBottom: '24px' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead><tr style={{ background: '#FAF0E6' }}>
                  ...
                  <tbody>
                    {PAYMENT_HISTORY.map(p => (
                      <tr key={p.id} style={{ borderBottom: '1px solid #F5EAF0' }}>
                        ...
                        <td style={{ padding: '12px 16px' }}><span style={{ fontSize: '11px', fontWeight: '700', padding: '3px 9px', borderRadius: '20px', background: '#d4edda', color: '#155724' }}>✅ {p.status}</span></td>
          ...
          {activePage === 'documents' && (
            <div style={{ background: 'white', border: '1px solid #D9C0CC', borderRadius: '12px', padding: '20px' }}>
              ...
              <div style={{ border: '2px dashed #D9C0CC', borderRadius: '10px', padding: '24px', textAlign: 'center', cursor: 'pointer', background: '#FAF0E6', marginBottom: '16px' }}
                onClick={() => { const newDoc = { id: `DOC-00${uploadedDocs.length + 1}`, name: `Uploaded Document ${uploadedDocs.length + 1}`, type: 'personal', date: new Date().toLocaleDateString(), size: '500 KB', icon: '📎' }; setUploadedDocs([...uploadedDocs, newDoc]); alert('✅ Document uploaded!'); }}>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                {['All', 'Receipts', 'Contracts', 'Personal'].map(f => (
                  <button key={f} onClick={() => setDocFilter(f)} style={{ padding: '6px 14px', borderRadius: '20px', border: '1.5px solid #D9C0CC', background: docFilter === f ? '#6B2D4E' : 'white', color: docFilter === f ? 'white' : '#6B2D4E', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>{f}</button>
                ))}
              </div>
              {uploadedDocs.map(d => (
                <div key={d.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', border: '1px solid #D9C0CC', borderRadius: '10px', marginBottom: '8px' }}>
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: '14px', marginBottom: '24px' }}>
                {[
                  ...
                  { label: 'ON-TIME', value: `${paidCycles}/${PAYMENT_HISTORY.length}`, icon: '✅' },
                  { label: 'CYCLES', value: paidCycles.toString(), icon: '🔄' },
              ...
              {editingProfile ? (
                <div>
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    <textarea value={profileData.bio} onChange={e => setProfileData({ ...profileData, bio: e.target.value })} placeholder="A few words about yourself..." rows={3} style={{ ...inp, resize: 'vertical' } as any} />
                  </div>
          ...
          {activePage === 'contact' && (
            <div style={{ background: 'white', border: '1px solid #D9C0CC', borderRadius: '12px', padding: '28px' }}>
              ...
              {contactSent && <div style={{ background: '#d4edda', border: '1px solid #c3e6cb', color: '#155724', padding: '12px 16px', borderRadius: '8px', marginBottom: '16px', fontSize: '13px' }}>✅ Message sent! The organizer will reply by email.</div>}
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <textarea value={contactMessage} onChange={e => setContactMessage(e.target.value)} placeholder="Write your message here..." rows={5} style={{ ...inp, resize: 'vertical' } as any} />
              </div>
              <button onClick={handleSendContact} disabled={!contactSubject || !contactMessage} style={btn({ width: '100%', padding: '12px', opacity: !contactSubject || !contactMessage ? 0.6 : 1 })}>
                📤 Send Message
              </button>
            </div>
          )}
          ...
          {activePage === 'comments' && (
            <>
              <div style={{ background: 'white', border: '1px solid #D9C0CC', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
                ...
                <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Share your experience..." rows={4} style={{ ...inp, marginBottom: '14px', resize: 'vertical' } as any} />
            ...
            <div style={{ background: 'white', border: '1px solid #D9C0CC', borderRadius: '12px', padding: '28px' }}>
              ...
              {ratingSent ? (
                ...
                <>
                  ...
                  <textarea value={ratingComment} onChange={e => setRatingComment(e.target.value)} placeholder="Additional comments (optional)..." rows={4} style={{ ...inp, marginBottom: '20px', resize: 'vertical' } as any} />
          ...
          {activePage === 'rules' && (
            <div style={{ background: 'white', border: '1px solid #D9C0CC', borderRadius: '12px', padding: '28px' }}>
              ...
              {[
                ...
                { title: '4. Confidentiality', text: 'Member identities are protected. Only TYN-IDs are visible to other members.' },
                { title: '5. Commission', text: 'The organizer charges 1% commission per distribution cycle.' },
                { title: '6. Withdrawal', text: 'Early withdrawal requires 30 days notice. Penalties may apply.' },
                { title: '7. Disputes', text: 'Disputes must be communicated in writing to the organizer within 7 days.' },
              ].map(r => (
              ...
              <button onClick={() => alert('Downloading contract PDF...')} style={btn({ width: '100%', padding: '12px' })}>📥 Download Contract PDF</button>
            </div>
          )}
            ...
            <div style={{ background: 'white', border: '1px solid #D9C0CC', borderRadius: '12px', padding: '28px' }}>
              ...
              {FAQ.map((f, i) => (
                <details key={i} style={{ marginBottom: '12px', border: '1px solid #D9C0CC', borderRadius: '10px', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '14px 18px', fontSize: '13px', color: '#7A5068', lineHeight: '1.6', borderTop: '1px solid #EDD9E5' }}>{f.a}</div>
                </details>
              ))}
          ...
          {activePage === 'privacy' && (
            <div style={{ background: 'white', border: '1px solid #D9C0CC', borderRadius: '12px', padding: '28px' }}>
              ...
              <button onClick={() => alert('✅ Privacy settings saved!')} style={btn({ marginTop: '20px', width: '100%', padding: '12px' })}>💾 Save Settings</button>
            </div>
          )}
          ...
          {activePage === 'invite' && (
            <div style={{ background: 'white', border: '1px solid #D9C0CC', borderRadius: '12px', padding: '28px' }}>
              ...
              <div style={{ background: '#EDD9E5', borderRadius: '10px', padding: '16px', marginBottom: '20px', textAlign: 'center' }}>
                ...
                <button onClick={() => { navigator.clipboard.writeText(`https://tarsyn-app.com/register?ref=${MEMBER_DATA.tynId}`); alert('✅ Link copied!'); }} style={{ ...btn(), marginTop: '12px' }}>📋 Copy Link</button>
              </div>
              <div style={{ marginBottom: '14px' }}>
              ...
              <button onClick={() => alert('✅ Invitation sent!')} style={btn({ width: '100%', padding: '12px' })}>📧 Send Invitation</button>
            </div>
          )}
