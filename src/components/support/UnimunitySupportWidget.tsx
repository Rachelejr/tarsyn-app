...
export default function UnimunitySupportWidget() {
  ...
  const panelStyle: CSSProperties = isMobile
    ...
    : tier === 'tablet'
      ? { right: 16, bottom: 16, width: `min(${panelWidth}px, calc(100vw - 32px))`, height: 'min(520px, calc(100vh - 32px))', borderRadius: 18 }
      : { right: 20, bottom: 20, width: `min(${panelWidth}px, 92vw)`, height: 'min(500px, calc(100vh - 40px))', borderRadius: 20 };
      ...
      >
        ...
        <div style={{ display: 'flex', borderTop: `1px solid ${C.border}`, flexShrink: 0, background: C.white }}>
          ...
          {!!user && (
            ...
            >
              {/* Per Rachele's instruction: the bottom nav tab keeps its
                  avatar icon and its onClick (still switches to the AI
