...
function systemPrompt(ctx: AIContext): string {
  ...
  return [
    ...
    'This chat displays your reply as plain text exactly as you write it - it does NOT render Markdown. So never use Markdown formatting characters in your reply: no "#" or "##" headers, no "**bold**" or "*italic*" asterisks, no "-" or numbered-list bullets, no backticks. Write in plain, professional sentences and short paragraphs instead, using line breaks (never symbols) to separate ideas. If you need to give a short numbered sequence of steps, write it as plain sentences like "First, ... Then, ... Finally, ..." rather than a "1. 2. 3." list.',
