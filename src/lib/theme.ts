// TARSYN — Palette officielle (4 couleurs)
// import { C } from '@/lib/theme';
// Ne pas ajouter de nouvelles couleurs sans validation.

export const C = {
  // Fond principal — page background
  ivoire: '#FFFDF7',

  // Fond cartes / sections
  creme: '#FBEEDD',

  // Accent principal — boutons, CTA, highlights
  or: '#E9C77B',

  // Accent froid — texte, liens secondaires, info, contraste
  bleu: '#9DBFD4',

  // Variantes utilitaires (nuances derivees des 4 couleurs, pas de nouvelles teintes)
  orFonce: '#C9A55E',      // hover/active boutons or
  orPale: '#F0DCA8',       // fonds legers or
  bleuFonce: '#6E93AC',    // texte fort contraste, fonds sombres
  bleuTresFonce: '#4A6B85',// sections tres sombres (hero, footer CTA)
  border: '#EAD9BE',       // bordures douces, derivees de la creme
  borderPale: '#F3E9D6',   // bordures tres legeres
};
